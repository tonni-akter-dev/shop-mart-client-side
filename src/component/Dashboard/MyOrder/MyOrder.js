...
const MyOrder = () => {
  ...
  const handleMyOrderDelete = (id) => {
    window.confirm("Are you sure you wish to delete this item?") &&
      axios.delete(`https://mighty-reef-87460.herokuapp.com/myOrderDelete/${id}`).then((res) => {
  ...
  return (
    <div>
      {products?.length === 0 ? (
        <h4 className=" text-pink-800 text-center">
          No order was found for you, please order first
        </h4>
      ) : (
        <h1 className=" text-pink-600 text-center">
          My Order {products?.length}{" "}
        </h1>
      )}

      {products?.length === 0 && (
        <img src="https://i.ibb.co/tq9K32W/Helium-10-xray.jpg" alt="" />
      )}
